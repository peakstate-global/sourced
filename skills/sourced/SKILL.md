...
- **O**pposed — argue the strongest case against a conclusion before presenting it, then map
  what the argument found: for every disagreement, give the conditions each side held under and
  say where each is right. The requirement is a **fresh thread, explicitly adversarial**, because an instance
  that is not holding the draft has no commitment to defend it. A different model is an upgrade, not the
  mechanism: it catches shared priors, where a fresh copy of yourself believes the same wrong
  thing. Record which grade ran and never report a lower grade as a higher one. Hunt places where
  one word covers two mechanisms, and for every claim that stands, state the observation that
  would make it false. **A claim that fails is not finished:** record where it holds, where
  it fails, and what is true instead in the failing region. Adversarial is the method;
...
- **E**valuated — anything reusable ships with three test cases, known answers, and a failure
  threshold. Triage a failing eval before believing it: mine reported 67% failure and it was the
  grader, three times running.
- **D**isclosed — a deliverable ships with the four-label provenance block: **Attribution,
  Accountable, Limitations, References**. Every line must either make a limit transparent or let
  ...
  padded to look diligent; `None material.` and `No external sources.` are each a complete line.
  Our own frameworks, estimates, arithmetic, predictions and judgement never appear there, not
  even to label them: they are the value of the piece, and INFERRED in the sidecar is the whole
  disclosure. An unfetchable source is "unavailable at the time of retrieval", with the blocked

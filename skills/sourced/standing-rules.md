# The seven standing rules, standalone

The same block that sits inside `SKILL.md`, byte for byte, for the case a skill cannot solve:
applying the standard when nothing has been invoked. Paste it into a system prompt, a project
instruction file, or the top of a thread. `skill_check.py` fails if the two copies drift apart.
